...
int main(){
    ...
    do{
        ...
        do{
            ...
            for(int i = 0; i < currentRow.length(); i++){
                if(currentRow[i] == '@'){
                    ...
                    if(neighbors < 4){
                        count++;
                        newCount++;
                        currentRow[i] = '.'; //was thinking about using a different character, but realized it doesn't matter when it gets removed
                    }
                }
                else{
                    //cout << " ";
            ...
            layout.GetCurrentNode()->SetRow(currentRow);
            //cout << endl;
        } while(layout.GetNext() != nullptr); //extremely rare sighting of a do while loop. take a photo
        cout << newCount << " rolls removed" << endl;
    } while(newCount != 0); //two in one program. tell your local wildlife magazine
    cout << "total rolls removed: " << count << endl;
